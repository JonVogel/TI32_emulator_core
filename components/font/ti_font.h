/*
 * TI-99/4A Default Character Set
 *
 * 8x8 pixel patterns for characters 32-127.
 * Each character is 8 bytes, one per row, MSB = leftmost pixel.
 * Patterns match the TI-99/4A console ROM default character set.
 *
 * Characters 128-255 are user-defined (CALL CHAR), initialized to 0.
 */
...
// Total characters in the set (0-255, but 0-31 are special)
#define TI_CHAR_COUNT 256
#define TI_CHAR_BYTES 8

static const uint8_t tiFont[TI_CHAR_COUNT][TI_CHAR_BYTES] PROGMEM =
{
  // Characters 0-31: undefined (blank)
  ...
  // Characters 32-95: Standard ASCII (TI ROM patterns)
  ...
  {0x00,0x00,0x10,0x00,0x10,0x00,0x00,0x00},  // 58 :
  {0x00,0x00,0x10,0x00,0x10,0x10,0x20,0x00},  // 59 ;
  ...
  {0x00,0x40,0x20,0x10,0x08,0x04,0x00,0x00},  // 92 backslash
  ...
  // Characters 96-127: Lowercase (TI Extended BASIC set)
  {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},  // 96 ` (grave, blank on TI)
  {0x00,0x00,0x38,0x04,0x3C,0x44,0x3C,0x00},  // 97 a
  {0x40,0x40,0x78,0x44,0x44,0x44,0x78,0x00},  // 98 b
  {0x00,0x00,0x38,0x44,0x40,0x44,0x38,0x00},  // 99 c
  {0x04,0x04,0x3C,0x44,0x44,0x44,0x3C,0x00},  // 100 d
  {0x00,0x00,0x38,0x44,0x7C,0x40,0x38,0x00},  // 101 e
  {0x18,0x24,0x20,0x78,0x20,0x20,0x20,0x00},  // 102 f
  ...
  {0x00,0x00,0x68,0x54,0x54,0x54,0x44,0x00},  // 109 m
  ...
  {0x00,0x34,0x58,0x00,0x00,0x00,0x00,0x00},  // 126 ~
  {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},  // 127 DEL

  // Characters 128-255: User-defined (initialized to blank)
  // CALL CHAR can redefine these at runtime
...
// ---------------------------------------------------------------------------
// V9T9-derived authentic TI-99/4A font, chars 32-127 only.
//
// Source: V9T9 emulator TIFONT.INC (chars 32-127, decimal → hex). V9T9 was
// the original TI-99/4A emulator and its font tracks the console GROM. The
// most visible difference from the default `tiFont` above is the lowercase
// block (97-122): in TI ROM these are *small caps* — 5-row uppercase shapes
// occupying rows 3-7 of the cell, not the PC-style lowercase forms.
//
// Chars 0-31 and 128-255 are intentionally not included here — TI BASIC
// leaves them user-redefinable. The active-font selector falls back to
// `tiFont`'s zero entries for those ranges.
// ---------------------------------------------------------------------------
static const uint8_t tiFontV9t9_32to127[96][TI_CHAR_BYTES] PROGMEM =
{
  ...
  { 0x00,0x38,0x54,0x50,0x38,0x14,0x54,0x38 }, // 36 '$'
  { 0x00,0x60,0x64,0x08,0x10,0x20,0x4C,0x0C }, // 37 '%'
  { 0x00,0x20,0x50,0x50,0x20,0x54,0x48,0x34 }, // 38 '&'
  { 0x00,0x08,0x08,0x10,0x00,0x00,0x00,0x00 }, // 39 '\''
  ...
  { 0x00,0x10,0x30,0x10,0x10,0x10,0x10,0x38 }, // 49 '1'
  { 0x00,0x38,0x44,0x04,0x08,0x10,0x20,0x7C }, // 50 '2'
  { 0x00,0x38,0x44,0x04,0x18,0x04,0x44,0x38 }, // 51 '3'
  { 0x00,0x08,0x18,0x28,0x48,0x7C,0x08,0x08 }, // 52 '4'
  { 0x00,0x7C,0x40,0x78,0x04,0x04,0x44,0x38 }, // 53 '5'
  { 0x00,0x18,0x20,0x40,0x78,0x44,0x44,0x38 }, // 54 '6'
  { 0x00,0x7C,0x04,0x08,0x10,0x20,0x20,0x20 }, // 55 '7'
  { 0x00,0x38,0x44,0x44,0x38,0x44,0x44,0x38 }, // 56 '8'
  { 0x00,0x38,0x44,0x44,0x3C,0x04,0x08,0x30 }, // 57 '9'
  { 0x00,0x00,0x30,0x30,0x00,0x30,0x30,0x00 }, // 58 ':'
  { 0x00,0x00,0x30,0x30,0x00,0x30,0x10,0x20 }, // 59 ';'
  { 0x00,0x08,0x10,0x20,0x40,0x20,0x10,0x08 }, // 60 '<'
  { 0x00,0x00,0x00,0x7C,0x00,0x7C,0x00,0x00 }, // 61 '='
  ...
  { 0x00,0x78,0x24,0x24,0x38,0x24,0x24,0x78 }, // 66 'B'
  { 0x00,0x38,0x44,0x40,0x40,0x40,0x44,0x38 }, // 67 'C'
  { 0x00,0x78,0x24,0x24,0x24,0x24,0x24,0x78 }, // 68 'D'
  { 0x00,0x7C,0x40,0x40,0x78,0x40,0x40,0x7C }, // 69 'E'
  { 0x00,0x7C,0x40,0x40,0x78,0x40,0x40,0x40 }, // 70 'F'
  { 0x00,0x3C,0x40,0x40,0x5C,0x44,0x44,0x38 }, // 71 'G'
  { 0x00,0x44,0x44,0x44,0x7C,0x44,0x44,0x44 }, // 72 'H'
  { 0x00,0x38,0x10,0x10,0x10,0x10,0x10,0x38 }, // 73 'I'
  { 0x00,0x04,0x04,0x04,0x04,0x04,0x44,0x38 }, // 74 'J'
  { 0x00,0x44,0x48,0x50,0x60,0x50,0x48,0x44 }, // 75 'K'
  { 0x00,0x40,0x40,0x40,0x40,0x40,0x40,0x7C }, // 76 'L'
  { 0x00,0x44,0x6C,0x54,0x54,0x44,0x44,0x44 }, // 77 'M'
  { 0x00,0x44,0x64,0x64,0x54,0x4C,0x4C,0x44 }, // 78 'N'
  { 0x00,0x7C,0x44,0x44,0x44,0x44,0x44,0x7C }, // 79 'O'
  ...
  { 0x00,0x38,0x44,0x40,0x38,0x04,0x44,0x38 }, // 83 'S'
  { 0x00,0x7C,0x10,0x10,0x10,0x10,0x10,0x10 }, // 84 'T'
  { 0x00,0x44,0x44,0x44,0x44,0x44,0x44,0x38 }, // 85 'U'
  { 0x00,0x44,0x44,0x44,0x28,0x28,0x10,0x10 }, // 86 'V'
  { 0x00,0x44,0x44,0x44,0x54,0x54,0x54,0x28 }, // 87 'W'
  ...
  { 0x00,0x00,0x10,0x28,0x44,0x00,0x00,0x00 }, // 94 '^'
  { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x7C }, // 95 '_'
  { 0x00,0x00,0x20,0x10,0x08,0x00,0x00,0x00 }, // 96 '`'
  { 0x00,0x00,0x00,0x38,0x44,0x7C,0x44,0x44 }, // 97 'a' (small cap)
  { 0x00,0x00,0x00,0x78,0x24,0x38,0x24,0x78 }, // 98 'b'
  { 0x00,0x00,0x00,0x3C,0x40,0x40,0x40,0x3C }, // 99 'c'
  { 0x00,0x00,0x00,0x78,0x24,0x24,0x24,0x78 }, // 100 'd'
  { 0x00,0x00,0x00,0x7C,0x40,0x78,0x40,0x7C }, // 101 'e'
  { 0x00,0x00,0x00,0x7C,0x40,0x78,0x40,0x40 }, // 102 'f'
  { 0x00,0x00,0x00,0x3C,0x40,0x5C,0x44,0x38 }, // 103 'g'
  { 0x00,0x00,0x00,0x44,0x44,0x7C,0x44,0x44 }, // 104 'h'
  { 0x00,0x00,0x00,0x38,0x10,0x10,0x10,0x38 }, // 105 'i'
  { 0x00,0x00,0x00,0x08,0x08,0x08,0x48,0x30 }, // 106 'j'
  { 0x00,0x00,0x00,0x24,0x28,0x30,0x28,0x24 }, // 107 'k'
  { 0x00,0x00,0x00,0x40,0x40,0x40,0x40,0x7C }, // 108 'l'
  { 0x00,0x00,0x00,0x44,0x6C,0x54,0x44,0x44 }, // 109 'm'
  { 0x00,0x00,0x00,0x44,0x64,0x54,0x4C,0x44 }, // 110 'n'
  { 0x00,0x00,0x00,0x7C,0x44,0x44,0x44,0x7C }, // 111 'o'
  { 0x00,0x00,0x00,0x78,0x44,0x78,0x40,0x40 }, // 112 'p'
  { 0x00,0x00,0x00,0x38,0x44,0x54,0x48,0x34 }, // 113 'q'
  { 0x00,0x00,0x00,0x78,0x44,0x78,0x48,0x44 }, // 114 'r'
  { 0x00,0x00,0x00,0x3C,0x40,0x38,0x04,0x78 }, // 115 's'
  { 0x00,0x00,0x00,0x7C,0x10,0x10,0x10,0x10 }, // 116 't'
  { 0x00,0x00,0x00,0x44,0x44,0x44,0x44,0x38 }, // 117 'u'
  { 0x00,0x00,0x00,0x44,0x44,0x28,0x28,0x10 }, // 118 'v'
  { 0x00,0x00,0x00,0x44,0x44,0x54,0x54,0x28 }, // 119 'w'
  { 0x00,0x00,0x00,0x44,0x28,0x10,0x28,0x44 }, // 120 'x'
  { 0x00,0x00,0x00,0x44,0x28,0x10,0x10,0x10 }, // 121 'y'
  { 0x00,0x00,0x00,0x7C,0x08,0x10,0x20,0x7C }, // 122 'z'
  { 0x00,0x18,0x20,0x20,0x40,0x20,0x20,0x18 }, // 123 '{'
  { 0x00,0x10,0x10,0x10,0x00,0x10,0x10,0x10 }, // 124 '|'
...
// ---------------------------------------------------------------------------
// Active font mode. Default: PC (today's font). Switched at runtime via
// CALL CHARSET("TI" | "PC"), persisted to NVS by the host.
// ---------------------------------------------------------------------------
enum TiFontMode : uint8_t
{
  TI_FONT_PC = 0,
  TI_FONT_TI = 1,
};
...
static void initCharPatterns()
{
  memcpy_P(charPatterns, tiFont, sizeof(tiFont));
  if (g_tiFontMode == TI_FONT_TI)
  {
    memcpy_P(&charPatterns[32], tiFontV9t9_32to127,
             sizeof(tiFontV9t9_32to127));
  }
}

// Reset chars 32-127 to ROM defaults of the active font mode.
// Used by `CALL CHARSET` from BASIC. Leaves chars 0-31 and 128-255 (the
// user-redefinable graphics block) untouched.
static void resetBasicChars()
{
  if (g_tiFontMode == TI_FONT_TI)
  {
    memcpy_P(&charPatterns[32], tiFontV9t9_32to127,
  ...
  {
    memcpy_P(&charPatterns[32], &tiFont[32], 96 * TI_CHAR_BYTES);
  }
}
